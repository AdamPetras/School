...
char* infix2postfix (const char* infExpr) {
	...
	for (unsigned i = 0; i <= stringSize; i++) {
		if (
			...
			// Je-li zpracovávanou položkou pravá závorka,
			// odebíram z vrcholu zasobniku položky a dávam
			// na konec výstupního řetězce až narazim na levou
			// závorku.
			untilLeftPar(s, postExpr, &postLen);
		} else if (infExpr[i] == '=') {
			// Je-li zpracovávanou položkou '=', pak postupně
			// odstraňuju prvky z vrcholu zásobníku a přidávam na
			// konec řetězce, až se zásobník zcela vyprázdní.
			// Na konec se přidá rovnítko, jako omezovač výrazu.
			while (!stackEmpty(s)) {
