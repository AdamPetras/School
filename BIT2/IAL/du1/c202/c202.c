 ...
 *	else
 *		return 0;
 *	Přepsáno do ternárního operátoru
*/
...
** funkci stackEmpty.
*/
	if (stackEmpty(s))		//pokud je zásobník prázdný zavolá fci error
		stackError(SERR_TOP);
	else
	{
		*c = s->arr[s->top];	//veme hodnotu zezhora zásobníku a přiřadí ju do proměnné
	}
...
** 
*/
	if(!stackEmpty(s))	//pokud neni prázdný zásobník tak dekrementuje top
		s->top--;
}
...
** funkci stackFull.
*/
	if(stackFull(s))	//pokud je plný zásobník zavolá error
	{
		stackError(SERR_PUSH);
	}
	else
	{
		s->arr[++s->top]=c;	//jinak inkrementuje top přiřadí navrch zásobníku hodnotu c
	}
}
