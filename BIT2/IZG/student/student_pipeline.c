...
void const *gpu_computeVertexAttributeDataPointer(
  ...
  assert(head != NULL);
  long unsigned int a = head->stride * (size_t)gl_VertexID; //krok čtecí hlavy * číslo vrcholu
  a += (size_t)head->buffer + head->offset; //dosavadní adresa += adresa bufferu + offset čtecí hlavy
  return (void const*)(a);
}
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  VertexIndex index = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation);
	for (long unsigned int i = 0; i < MAX_ATTRIBUTES; i++) 
  {
    ...
    {
			output->attributes[i] = gpu_computeVertexAttributeDataPointer(head, index); //získání adresy jednotlivého atributu
		}
  }
}
...
void gpu_runPrimitiveAssembly(
  ...
  GPUVertexPullerOutput outputpuller;
  GPUVertexShaderInput inputpuller;

  for (long unsigned int i = 0; i < nofPrimitiveVertices; i++) 
  {
	  gpu_runVertexPuller(&outputpuller, puller, baseVertexShaderInvocation + i); //spustení vertex pulleru a shaderu nad daným vrcholem
	  inputpuller.gl_VertexID = gpu_computeGLVertexID(puller->indices, baseVertexShaderInvocation + i); //výpočet vertexu
	  inputpuller.attributes = &outputpuller; //nastavení atributů
	  vertexShader(&(primitive->vertices[i]), &inputpuller, gpu); //vložení do parametru primitive
  }
primitive->nofUsedVertices = nofPrimitiveVertices;  //nastavení nofprimitiveVert... do primitiva
}
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  assert(lines != NULL);
  float temprary = 1.0f /((vertices[1].data[1] - vertices[2].data[1]) * (vertices[0].data[0] - vertices[2].data[0]) +
	  (vertices[2].data[0] - vertices[1].data[0]) * (vertices[0].data[1] - vertices[2].data[1]));
  coords->data[0] = ((pixelCenter[0].data[0] - vertices[2].data[0])*(vertices[1].data[1] - vertices[2].data[1]) + (pixelCenter[0].data[1] - vertices[2].data[1])*(vertices[2].data[0] - vertices[1].data[0])) * temprary;
  coords->data[1] = ((pixelCenter[0].data[0] - vertices[2].data[0])*(vertices[2].data[1] - vertices[0].data[1]) + (pixelCenter[0].data[1] - vertices[2].data[1])*(vertices[0].data[0] - vertices[2].data[0])) * temprary;
